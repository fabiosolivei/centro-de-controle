...
Parses OpenClaw logs for token usage and pushes to the API.
Budget tracking is handled by the backend via Langfuse.
...
# GPT-5.2 pricing (USD per 1M tokens)
# Source: OpenAI API pricing / openclaw.json.template
PRICING = {
    ...
    "default": {"input": 1.75, "output": 14.00, "cached_input": 0.175},
}
        ...
        if env_file.exists():
            for line in env_file.read_text().splitlines():
                if line.startswith("ATLAS_PUSH_KEY="):
                    ...
                    break
        log(f"Loaded secrets (atlas={'yes' if ATLAS_PUSH_KEY else 'no'})")
    except Exception as e:
        log(f"Error loading secrets: {e}")


# Balance tracking is now handled by the backend via Langfuse.
# The cost-collector only parses OpenClaw logs for token usage.
...
def parse_openclaw_logs() -> list[dict]:
    ...
    the primary cost tracking mechanism is through Langfuse (via LiteLLM).
...
def main():
    ...
    # Push to Centro de Controle API
    if calls:
        success = push_to_api(None, calls)
        if success:
            log(f"Done: {len(calls)} calls pushed")
        else:
            log("Failed to push data")
    else:
        log("No new calls to push")
